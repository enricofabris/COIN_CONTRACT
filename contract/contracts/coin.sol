...
// player set his bet (0 or 1), his bet amount and send that amount to the contract
   function setBet(uint amount, uint bet) public payable costs( (players[msg.sender].amount)*100 ){
       ...
       require((players[msg.sender].amount)*100 >= minBet*100, "Bet under minimum bet");
// require that the amount bet is lower than half balance
   ...
   function __callback(bytes32 _queryId, string memory _result, bytes memory proof) public {
       ...
       players[playerId[_queryId].playerAddress].playerBalance = players[playerId[_queryId].playerAddress].playerBalance + ( (playerId[_queryId].amount)*100*2 );
       ...
       players[playerId[_queryId].playerAddress].moneyWon = players[playerId[_queryId].playerAddress].moneyWon + (playerId[_queryId].amount)*100;

      TotGamesWon = 1 + TotGamesWon;
      TotMoneyWon = (playerId[_queryId].amount)*100 + TotMoneyWon;
       ...
       players[playerId[_queryId].playerAddress].moneyBetted = players[playerId[_queryId].playerAddress].moneyBetted + (playerId[_queryId].amount)*100;
       //maxBet = balance/2;
...
function getResult() public view returns(uint amount, uint bet, string memory message, uint result, uint playerBalance, uint games, uint moneyBetted, uint gamesWon, uint moneyWon){
   ...
   function depositFunds(uint deposit) public payable onlyOwner costs( deposit*100 ){
...
// only owner can withdraw money
  ...
  function withdrawFundsPlayer(uint withdraw) public {
      ...
      players[msg.sender].playerBalance = players[msg.sender].playerBalance - (withdraw*100);
      msg.sender.transfer(withdraw*100);
